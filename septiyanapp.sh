...
if [ "$#" -eq 0 ] || [ $1 = "-h" ] || [ $1 = "--help" ]; then
    echo "Usage: ./septiyanapp [OPTIONS] COMMAND [arg...]"
    echo "       ./septiyanapp [ -h | --help ]"
    echo ""
    echo "Options:"
...
stop_existing() {
  MYAPP="$(docker ps --all --quiet --filter=name=septiyanapp)"
  REDIS="$(docker ps --all --quiet --filter=name=septiyanapp-redis)"
  MONGO="$(docker ps --all --quiet --filter=name=septiyanapp-mongo)"
